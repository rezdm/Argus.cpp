...
destination::destination(int sort_val, const std::string& name_val, int timeout_val, int warning_val, int failure_val, int reset_val, int interval_val, int history_val, test_config test_val)
    : sort_(sort_val), name_(name_val), timeout_(timeout_val), warning_(warning_val), failure_(failure_val), reset_(reset_val), interval_(interval_val), history_(history_val), test_(std::move(test_val)) {
    validate_parameters();
}

void destination::set_sort(const int sort_val) {
    sort_ = sort_val;
}
...
bool destination::is_valid() const {
