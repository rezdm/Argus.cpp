...
void monitor_state::update_status(const bool test_success) {
  if (test_success) {
    ...
    if (current_status_ != monitor_status::ok && consecutive_successes_ >= destination_.get_reset()) {
...
const std::string& monitor_state::get_group_name() const { return group_.get_group_name(); }

std::string monitor_state::get_test_description() const { return test_description_; }
