...
void test_result::set_success(const bool success_val) {
    success_ = success_val;
}
...
void test_result::set_timestamp(const std::chrono::system_clock::time_point& timestamp_val) {
    timestamp_ = timestamp_val;
}

void test_result::set_error(const std::string& error_val) {
    error_ = error_val;
}

void test_result::clear_error() {
    error_.reset();
}

bool test_result::is_valid() const {
