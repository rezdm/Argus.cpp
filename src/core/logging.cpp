...
void logger::log_startup(const std::string& config_path) { spdlog::info("Starting {} Monitor with config: {}", constants::APPLICATION_NAME, config_path); }

void logger::log_shutdown() { spdlog::info("Shutting down {} Monitor", constants::APPLICATION_NAME); }

void logger::log_memory_usage(const std::string& phase) { spdlog::info("Memory [{}]: Phase completed", phase); }

void logger::log_config_loaded(const std::string& instance_name) { spdlog::info("Loaded configuration for instance: {}", instance_name); }

void logger::log_config_reload_start(const std::string& config_path) { spdlog::info("Starting configuration reload from: {}", config_path); }

void logger::log_config_reload_success() { spdlog::info("Configuration reload completed successfully"); }

void logger::log_config_reload_failure(const std::string& error) { spdlog::error("Configuration reload failed: {}. Continuing with current configuration.", error); }

void logger::log_component_start(const std::string& component_name) { spdlog::info("{} started successfully", component_name); }

void logger::log_component_stop(const std::string& component_name) { spdlog::info("Stopping {} for reload", component_name); }
...
void logger::log_performance(const std::string& operation, long duration_ms) { spdlog::debug("{} completed in {}ms", operation, duration_ms); }
